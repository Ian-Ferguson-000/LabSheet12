...
void Game::checkKeyboardState()
{
	...
	{
		m_myPlayer.increaseSpeed(); // the player’s movement speed should increase by 1.
	}

	if (sf::Keyboard::isKeyPressed(sf::Keyboard::Key::S))
	{
		m_myPlayer.reduceSpeed(); // the player’s movement speed should decrease by 1.
	}

	if (sf::Keyboard::isKeyPressed(sf::Keyboard::Key::I))
	{
		m_myPlayer.increaseSize(); // the size of the player's image increases. This occurs only once.
	}

	if (sf::Keyboard::isKeyPressed(sf::Keyboard::Key::C))
	{
		m_myPlayer.changeColour(); // changes the colour of the player's image when C is pressed
	}

	if (sf::Keyboard::isKeyPressed(sf::Keyboard::Key::D))
	{
		m_myPlayer.changeDirection();// changes the direction the player is moving when D is pressed
	}
}
...
void Game::update(sf::Time t_deltaTime)
{
	...
	m_myPlayer.move(); // calls the player object to move itself in the appropriate direction
